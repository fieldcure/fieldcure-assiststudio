...
**Lightweight document text extraction for .NET** — DOCX, HWPX, and more. Tables are converted to markdown for LLM / RAG consumption.
...
- **Extensible** — Implement `IDocumentParser` to add new formats
...
foreach (var ext in DocumentParserFactory.SupportedExtensions)
    Console.WriteLine(ext);  // .docx, .hwpx
```
...
Implement `IDocumentParser` and register it in `DocumentParserFactory`:
